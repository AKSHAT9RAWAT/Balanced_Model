...
def run_captioning(temp_input, batch_size=5):
    ...
    # Patch caption.py to use temp_input and FINAL_DIR
    # We'll set environment variables for base_folder and output_folder
    env = os.environ.copy()
...
def get_latest_cluster_caption_json():
    files = [f for f in os.listdir(FINAL_DIR) if f.startswith("cluster_captions_batch_") and f.endswith(".json")]
    files.sort()
    return os.path.join(FINAL_DIR, files[-1]) if files else None

def prepare_trigger_input(cluster_json, trigger_input_json):
    print_step("Step 2: Preparing input for GloVe trigger model...")
...
def run_trigger_model(trigger_input_json, trigger_output_json):
    print_step("Step 3: Running GloVe trigger model for album classification...")
    env = os.environ.copy()
...
def run_notification(trigger_output_json, notification_output_json):
    print_step("Step 4: Generating notifications for label 1 albums...")
    subprocess.run([
...
def main():
    ...
    # 4. Prepare input for trigger model
    cluster_json = get_latest_cluster_caption_json()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    trigger_input_json = os.path.join(FINAL_DIR, f"trigger_input_{timestamp}.json")
    prepare_trigger_input(cluster_json, trigger_input_json)
    # 5. Run trigger model
    trigger_output_json = os.path.join(FINAL_DIR, f"trigger_output_{timestamp}.json")
    run_trigger_model(trigger_input_json, trigger_output_json)
    # 6. Run notification
    notification_output_json = os.path.join(FINAL_DIR, f"notification_output_{timestamp}.json")
    run_notification(trigger_output_json, notification_output_json)
    # 7. Cleanup temp input
    shutil.rmtree(temp_input)
    print_step("Pipeline complete! All outputs are in the FINAL folder.")
    print(f"\n- Captions: {cluster_json}\n- Trigger input: {trigger_input_json}\n- Trigger output: {trigger_output_json}\n- Notifications: {notification_output_json}\n")
